...
if __name__ == "__main__":
  ...
  flags.DEFINE_string(
      "model", "LogisticModel", "Which Model to use: see cvd_models.py")
...
def get_input_evaluation_tensors(reader,
  ...
  Returns:
    A tuple containing the ids, images, and labels
...
def build_graph(reader,
  ...
  image_id_batch, model_input_raw, labels_batch = get_input_evaluation_tensors(  # pylint: disable=g-line-too-long
      reader,
      eval_data_pattern,
      batch_size=batch_size)
  ...
  tf.add_to_collection("id_batch", image_id_batch)
  tf.add_to_collection("labels", tf.cast(labels_batch, tf.float32))
  tf.add_to_collection("summary_op", tf.summary.merge_all())


def evaluation_loop(image_id_batch, prediction_batch, label_batch, loss,
                    summary_op, saver, summary_writer, evl_metrics,
                    last_global_step_val):
  """Run the evaluation loop once.

  Args:
    image_id_batch: a tensor of image ids mini-batch.
  ...
  with tf.Session() as sess:
    ...
    fetches = [image_id_batch, prediction_batch, label_batch, loss, summary_op]
